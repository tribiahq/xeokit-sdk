...
function clusterizeV2 (entities, meshes) {
    ...
    for (let i = 0; i < numEntities; i++) {
        ...
        var aabbEntity = math.collapseAABB3();
        var numTriangles = 0
        ...
        entity.meshIds.forEach (meshId => {
            ...
            aabbEntity[0] += mesh.origin[0];
            aabbEntity[1] += mesh.origin[1];
            aabbEntity[2] += mesh.origin[2];
            aabbEntity[3] += mesh.origin[0];
            aabbEntity[4] += mesh.origin[1];
            aabbEntity[5] += mesh.origin[2];
    ...
    {
        var generateClustersResult = makeClusters ({
            aabbTree: (rTreeBasedAabbTree = generateAABB (_aabbsForIndexes)),
        });
